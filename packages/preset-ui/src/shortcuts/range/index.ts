...
const getRangeSlideShortcuts = () => {
    const ranges = {
        "moz-range-slide-thumb":
            `relative z-[1] appearance-none rounded-full ring-2 ring-current size-[--ui-range-thumb-size] 
            mt-[calc(var(--ui-range-mt)*-1)] border-0 bg-[--ui-range-thumb-bg]`,
        "moz-range-slide-track":
            `group-disabled:bg-opacity-50 bg-[--ui-range-track-bg] h-[--ui-range-track-height] rounded-lg`,
        'ui-input-range': `w-full absolute appearance-none bg-transparent
                     disabled:cursor-not-allowed disabled:opacity-50
                     range-slider-thumb:ring-2 
